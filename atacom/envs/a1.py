...
class A1EffVel(A1Walking):
    def __init__(self, num_envs, horizon, headless, domain_randomization=True, camera_position=(105, 0, 4), camera_target=(95, 0, 0), action_scale=1.5, Kp=1., Kd=0., Ki=0.1):
        ...
        self._last_joint_vel = torch.zeros((num_envs, self.NUM_JOINTS), device=self._device)
        self._integral_error = torch.zeros((num_envs, self.NUM_JOINTS), device=self._device)

        self.action_ratio = torch.ones((num_envs, self.NUM_JOINTS), device=self._device)
        ...
        high_gains = [5., 5., 5.]
        state_gains = [1.]
        self.disturbance_gains = [1., 1.]

        self.observers = [MixedObserver(num_envs, self.NUM_JOINTS, self.dt, self._device, h_g, s_g, d_g) for h_g, s_g, d_g in zip(high_gains, state_gains, self.disturbance_gains)]

        self.observer_plotter = Plotter(data_dim=len(self.observers) + 1, data_len=100, n_row=4, n_col=3, title="observer", path="plot/observer", data_labels=["state", *[f'hg_{h_g}_sg_{s_g}_dg_{d_g}' for h_g, s_g, d_g in zip(high_gains, state_gains, self.disturbance_gains)]])
        self.disturbance_plotter = Plotter(data_dim=len(self.observers) + 1, data_len=50, n_row=4, n_col=3, title="disturbance", path="plot/observer", data_labels=["velocity", *[f'hg_{h_g}_sg_{s_g}_dg_{d_g}' for h_g, s_g, d_g in zip(high_gains, state_gains, self.disturbance_gains)]])
    ...
    def _reapeat_dof(self, value):
        if self.NUM_JOINTS % len(value) != 0:
            raise ValueError(f"Invalid gains size: {len(value)}")
        repeat_len = self.NUM_JOINTS // len(value)
...
class A1Atacom():
    def __init__(self, cfg):
        ...
        self.env_info['n_joints'] = self.NUM_JOINTS
